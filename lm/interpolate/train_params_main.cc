...
void train_params(
  ...
  std::size_t context_size=5; // (context_size+1)-grams considered in perplexity
  double stepdecreasefactor=0.1; // if step unsuccessful
  double initstepsize = 1.0; // Initial step size
  std::size_t linesinstartercorpus = 12; // The first few lines are tuned first, to find basin of attraction for Newton
  ...
  // Find the location of the token <s>, which might be given unnaturally large probability
  std::size_t beginofsentence_idx = vocab.size(); // unreachable index
  for (std::size_t i = 0; i < vocab.size(); ++i) 
    if (vocab[i].compare("<s>")==0)
    {
      beginofsentence_idx = i;
      cerr << " Beginning-of-sentence token at index " << beginofsentence_idx << endl;
      break;
    }

  for (std::size_t setiter=0; setiter<linestotune.size(); ++setiter)
  {
    cerr << " Now tuning the first " << linestotune[setiter] << " lines" << endl;
    ...
    for (int iter = 0; iter < ITERATIONS; ++iter) { // iterations
      ...
      vector<string> context(context_size, "<s>"); // Hard-coded to be (context_size+1)-gram perplexity

      double ppl = 0.0;
      DVector grad = DVector::Zero(nlambdas);
      DMatrix H = DMatrix::Zero(nlambdas, nlambdas);
      cerr << "o";
      for (std::size_t ci = 0; ci < linestotune[setiter]; ++ci) { // sentences in tuning corpus
        const vector<string>& sentence = corpus[ci];
        // Reset the context at the beginning of the line.
        std::fill(context.begin(), context.end(), "<s>");
        for (std::size_t t = 0; t < sentence.size(); ++t) { // words in sentence
          if (sentence[t].compare("<s>")==0)
            cerr << " <s> found at sentence " << ci << ", word " << t << endl;
          ...
          // Allows us to avoid overflow with negative params, could choose maxlogprob = -999.0 if we wanted better accuracy, at expense of speed

          DVector expectfeats      = DVector::Zero(nlambdas);
          DMatrix expectfeatmatrix = DMatrix::Zero(nlambdas, nlambdas);
          for (std::size_t i = 0; i < vocab.size(); ++i) { // probs over possible n-grams, for normalization
            // Do not include the beginning-of-sentence token <s>. This token is given an inappropriately
            // large probability (for this computation, at least) by lmplz.
            if (i!=beginofsentence_idx)
            {
          ...
          // Remove begin-of-sentence probability from normalization, since it doesn't belong
          //z -= 1.0; // This was wrong (z is scaled here, so 1 is not right. Assumed begin-of-sent was logprob 0.0), corrected above
      ...
      if ((ppl<bestppl) || (iter==0))
      {
        ...
        cerr << "Change in weights from best, step size " << stepsize << ": " << endl;
        cerr << deltaparams << endl;
      }
      else
      {
        ...
        cerr << "No improvement, so taking smaller step. Change in weights from best, step size " << stepsize << ":" << endl;
        cerr << -bestgrad*stepsize << endl;
